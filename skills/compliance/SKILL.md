---
name: compliance
description: SOC2 and HIPAA compliance auditing, implementation, and continuous monitoring. Scans codebases for control gaps, generates policies, and produces auditor-ready reports.
---
...
```bash
skills/compliance/audit.sh <framework> [target-path]
```
...
```bash
skills/compliance/implement.sh <framework> [target-path]
```
...
```bash
skills/compliance/monitor.sh <framework> [target-path]
```
...
```json
{
  ...
  "schedule": "0 9 * * 1",
  "type": "agent",
  "job": "Run compliance monitoring for all frameworks: skills/compliance/monitor.sh all . Then summarize any DRIFT or VIOLATION findings and create issues for remediation.",
  "enabled": true
}
```
...
1. Create `skills/compliance/frameworks/<framework>.md` with control requirements
