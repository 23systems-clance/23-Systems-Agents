...
description: "Sync with Google NotebookLM — create notebooks, add sources (URLs, YouTube, files), chat with sources, and generate artifacts (audio podcasts, videos, reports, quizzes, slide decks, infographics). Use for research synthesis, content generation, and source-grounded Q&A."
---
...
1. **Install** (run once, or use `skills/sync-notebooklm/install.sh`):
   ```bash
...
```bash
skills/sync-notebooklm/install.sh
```
...
HTML report templates are available in `skills/sync-notebooklm/templates/` for generating polished, branded output from NotebookLM research. These use the SOP template framework with the 23 Systems brand preset (midnight).

### ICP Report

Template: `skills/sync-notebooklm/templates/icp-report-template.html`
Output directory: `references/reports/`

**Workflow — this is the standard flow for all ICP research:**

1. **Research the target company** — use `search-web` skill or `skills/search-web/content.js` to extract content from:
   - Company homepage
...
5. **Produce the branded HTML report** using the ICP report template at `skills/sync-notebooklm/templates/icp-report-template.html`. Fill in all 10 standard sections using data from the research + NotebookLM briefing. Save to:
...
- Always run `skills/sync-notebooklm/install.sh` before using NotebookLM commands in a job.
