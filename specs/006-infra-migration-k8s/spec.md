...
| **Agent execution** | Microsandbox microVMs (Mac Mini) | Cloud Run Jobs | K8s Jobs (GKE) |
...
A user (or cron/trigger) creates a job via the event handler. Instead of pushing a `job/*` branch to trigger a GitHub Actions workflow, the event handler enqueues the job to BullMQ. A worker process picks it up, spawns a microsandbox microVM with the agent (using the same OCI images as Docker), and the sandbox runs the job exactly as it does today — cloning the repo, running the LLM agent, committing results, and creating a PR.
...
2. **Given** a job is in the BullMQ queue, **When** a worker picks it up, **Then** a microsandbox microVM is spawned with the correct env vars (SECRETS, LLM_SECRETS, LLM_MODEL, etc.)
...
- What happens when a job's microsandbox runs out of memory? Worker catches OOM exit code, sends failure notification, respects per-tenant resource limits.
...
- **FR-002**: System MUST spawn microsandbox microVMs (OCI-compatible) from BullMQ worker process with identical env var injection (SECRETS, LLM_SECRETS, LLM_MODEL, LLM_PROVIDER, REPO_URL, BRANCH, OPENAI_BASE_URL, AGENT_BACKEND)
...
| 2 | **Agent execution** | `run-job.yml` spins up Docker container on GH runner | Worker pulls from BullMQ, spawns microsandbox microVM locally (Phase 0) or K8s Job (Phase 1+) | 0 |
...
| 20 | **Concurrency control** | GitHub's 20 parallel jobs limit | BullMQ concurrency per queue. Default: 2 concurrent sandboxes (16GB RAM constraint — ~5GB available after OS + Ollama + Redis + event handler; no Docker Desktop VM overhead). Per-tenant limits in Phase 2 | 0 |
...
| **RAM budget** | ~3GB OS + ~1GB Redis/Next.js + ~9GB Phi-4 + ~3GB headroom = 16GB (no Docker Desktop VM overhead — microsandbox runs directly on Apple Hypervisor) |
...
│  │  ├── Spawns microsandbox microVMs    │                    │
│  │  ├── Monitors sandbox exit           │                    │
...
Step 2 [DETERMINISTIC] ─── Spawn Sandbox
  ├── microsandbox.create() with OCI image, env vars, resource limits
...
Step 7 [DETERMINISTIC] ─── Notify + Meter
  ...
  └── Clean up (destroy sandbox, update job status in BullMQ)
```
...
const worker = new Worker('agent-jobs', async (job) => {
  ...
  // Step 2: Spawn Sandbox [DETERMINISTIC]
  emit('spawn', 'started');
  const sandbox = await spawnSandbox(image, env, limits); // microsandbox microVM
  const timeout = setTimeout(() => killSandbox(sandbox, 'timeout'), limits.maxTime);
  emit('spawn', 'done');

  // Step 3: Agent Execution [AGENTIC]
  emit('execute', 'started');
  let exitCode = await waitForExit(sandbox);
  ...
  if (exitCode === 0) {
    ...
    if (!validation.passed && config.retries_remaining > 0) {
      emit('retry', 'started');
      exitCode = await retryWithContext(sandbox, image, env, validation.errors);
...
- [ ] Agent microsandbox microVMs spawn and execute identically to current behavior
...
### Risk 3: Microsandbox Maturity
**Impact:** Medium — microsandbox is v0.1.x (~1 year old), less battle-tested than Docker
**Probability:** Low-Medium — OCI image compatibility is proven, but edge cases may exist
**Mitigation:** Microsandbox runs standard OCI images (same images as Docker). Each sandbox gets a dedicated kernel via libkrun microVM — stronger isolation than Docker containers. Runs directly on Apple Hypervisor on M4 (no Docker Desktop Linux VM overhead). Fallback: swap `worker/sandbox.js` to use `docker run` if microsandbox has issues — the interface is identical (spawn, monitor, kill).
...
- [ ] Blueprint worker process (separate PM2 process) — 7-step state machine that pulls jobs and spawns microsandbox microVMs
...
3. Add `bullmq` and `microsandbox` dependencies to event handler
...
7. Implement sandbox spawning in worker via microsandbox Node.js SDK (same env vars as `run-job.yml`) with timeout watchdog, default concurrency: 2
...
6. Modify worker to use Cloud Run Jobs API instead of microsandbox SDK
...
| `spawn-agent-sandbox` | Pull from BullMQ, spawn microsandbox microVM with secrets injection | 0 |
...
| `monitor-worker-health` | Check worker, Redis, microsandbox runtime, Ollama status (heartbeat integration) | 0 |
