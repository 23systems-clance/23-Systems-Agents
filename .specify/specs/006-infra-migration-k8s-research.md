...
|----------|---------|-----------|
| `run-job.yml` | `job/*` branch creation | Sparse checkout → read `job.config.json` → build SECRETS/LLM_SECRETS JSON → select Docker image → `docker run` with env vars |
...
Passed from `run-job.yml` to Docker containers:
...
|-----------|---------------|
| Entrypoint scripts | Same env vars, same container, same behavior |
...
3. **Worker process crash recovery**: BullMQ has stalled job detection, but container orphaning is possible if the worker dies mid-execution.
