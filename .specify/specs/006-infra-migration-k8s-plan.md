...
**Primary Dependencies**: Next.js 14, 23wf npm package, Drizzle ORM, BullMQ (new), ioredis (new), node-postgres/postgres.js (new), Ollama (local)
...
**Target Platform**: Mac Mini M4 base (16GB RAM), Docker, PM2 process manager
...
**Constraints**: 16GB RAM budget (~9GB Ollama + ~3GB OS + ~2GB Docker/Redis/Next.js + ~2GB headroom), max 2 concurrent agent containers
...
| **2. Failure Awareness** | PASS | Every step has failure handling. BullMQ stalled job detection. Container timeout watchdog. Cloud fallback for Ollama |
...
**Key insight**: Even with BullMQ, we still need the GitHub branch — the container clones it. The difference is that `run-job.yml` no longer triggers. The branch is just a data carrier.
...
```text
.specify/specs/
├── 006-infra-migration-k8s.md           # Spec (clarified)
├── 006-infra-migration-k8s-research.md  # Research findings
├── 006-infra-migration-k8s-data-model.md # Database schema
├── 006-infra-migration-k8s-plan.md      # This file
└── 006-infra-migration-k8s-tasks.md     # Task list (created by /speckit.tasks)
```
...
├── container.js                # Docker container spawn/monitor
...
Clusters/config/instrumentation.js # Add BullMQ init + worker start
...
4. **Step 2: Spawn Container** — `docker run` with env vars, timeout watchdog (default 30 min)
5. **Step 3: Agent Execution** — Wait for container exit, stream logs
6. **Step 4: Validate Output** — Check exit code, find PR, run lint on changed files, anomaly detection
7. **Step 5: Retry** — If validation failed, feed errors back, re-run container (max 1 retry), label `[needs-review]` on 2nd failure
...
**Exit criteria**: Full job lifecycle works — chat → enqueue → worker picks up → container runs → PR created → auto-merged → notification sent. Job status API returns real-time BullMQ state.
...
- **Resource isolation** — worker's Docker container spawning doesn't compete with Next.js request handling
...
| Docker container orphaning | Worker kills container on stalled job detection | `docker ps --filter ancestor=...` cleanup script |
| 16GB RAM exhaustion | Limit to 2 concurrent containers, monitor with `htop` | Reduce to 1 concurrent if needed |
